...
import { Usuario, Unidade, Perfil } from '@/types';
...
function Dashboard() {
  const stats = [
    { label: 'Usuarios',  valor: db.getUsuarios().length },
    { label: 'Coordenad.',valor: db.getUsersByPerfil('coordenador').length },
    { label: 'Instrutores',valor: db.getUsersByPerfil('instrutor').length },
    { label: 'Unidades',  valor: db.getUnidades().length },
    { label: 'Salas',     valor: db.getSalas().length },
    { label: 'Turmas',    valor: db.getTurmas().length },
    { label: 'Reservas',  valor: db.getReservas().length },
    { label: 'Chaves',    valor: db.getChaves().length },
  ];
  return (
    <View>
      ...
      <Button title="Resetar dados de exemplo" variant="ghost" icon="refresh-outline"
        onPress={() => Alert.alert('Resetar', 'Isso apagara tudo e recarregara os dados de exemplo. Continuar?', [
          { text: 'Cancelar', style: 'cancel' },
          { text: 'Resetar', style: 'destructive', onPress: () => db.resetarTudo() },
        ])} />
    </View>
  );
}
...
function Usuarios({ onChange }: { onChange: () => void }) {
  ...
  const excluir = (u: Usuario) => Alert.alert('Excluir', `Excluir ${u.nome}?`, [
    { text: 'Cancelar', style: 'cancel' },
    { text: 'Excluir', style: 'destructive', onPress: async () => { await db.delUser(u.id); onChange(); } },
  ]);
...
function Unidades({ onChange }: { onChange: () => void }) {
  ...
  const excluir = (u: Unidade) => Alert.alert('Excluir', `Excluir ${u.nome}?`, [
    { text: 'Cancelar', style: 'cancel' },
    { text: 'Excluir', style: 'destructive', onPress: async () => { await db.delUnidade(u.id); onChange(); } },
  ]);
  return (
    <View>
      ...
      {list.map(r => {
        ...
        return (
          <Card key={r.id} style={{ marginBottom: 10 }}>
            ...
            <Text style={styles.sub}>{r.diasSemana.map(d => d.toUpperCase()).join(', ')}</Text>
...
const styles = StyleSheet.create({
  ...
  statValor: { fontSize: 26, fontWeight: '800', color: Colors.primary },
